...
public class MatrixComponent extends JPanel {
    BufferedImage image;
    boolean scaleMax = true;
    boolean scaleMin = false;
    ...
    public void setScale(boolean min, boolean max) {
        this.scaleMin = min;
        this.scaleMax = max;
    }
    ...
    public synchronized void setMatrix( D1Matrix64F A ) {
        ...
        double minValue = CommonOps.elementMinAbs(A);
        renderMatrix(A,image,minValue, maxValue);
        repaint();
    }

    public void renderMatrix( final D1Matrix64F M , final BufferedImage image , double minValue, double maxValue )
    {
        final int w = image.getWidth();
        final int h = image.getHeight();

        final double widthStep = (double)M.numCols / image.getWidth();
        final double heightStep = (double)M.numRows / image.getHeight();

        if (!scaleMin) minValue = 0;
        if (!scaleMax) maxValue = 1.0;
